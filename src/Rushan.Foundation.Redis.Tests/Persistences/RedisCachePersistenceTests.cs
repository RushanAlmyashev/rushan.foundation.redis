...
namespace Rushan.Foundation.Redis.Tests.Persistences
{
    [TestFixture]
    ...
    public class RedisCachePersistenceTests
    {
        ...
        public void SetUp()
        {
            ...
            _connectionString = "127.0.0.1:6379,password=redispw,abortConnect=false,defaultDatabase=0,syncTimeout=3000";
        ...
        [Test]
        public void WhenCallStringIncrement_ReturnExpectedValue()
        {
            var key = Guid.NewGuid().ToString();

            _target.StringIncrement(key);
            _target.StringIncrement(key);
            _target.SetCacheKeyExpire(key, TimeSpan.FromSeconds(10));
            
            var byteArrayValue = _target.GetCachedValue(key);
            var actual = Convert.ToInt32(System.Text.Encoding.UTF8.GetString(byteArrayValue));

            Assert.AreEqual(2, actual);
        }

        [Test]
        public async Task WhenCallStringIncrementAsync_ReturnExpectedValue()
        {
            var key = Guid.NewGuid().ToString();

            await _target.StringIncrementAsync(key);
            await _target.StringIncrementAsync(key);
            await _target.SetCacheKeyExpireAsync(key, TimeSpan.FromSeconds(10));

            var byteArrayValue = await _target.GetCachedValueAsync(key);
            var actual = Convert.ToInt32(System.Text.Encoding.UTF8.GetString(byteArrayValue));
